...
async function loadInitialData(sClass) {
    ...
    try {
        ...
        document
            ...
            if (currentAddress > startDate && currentDate < endDate) {
...
function generateCountDown(ele, claimDate) {
    clearInterval(countDownGlobal);
    //SET THHHE DATE WERE COUNTING DOWN TO
    var countDownDate = new Date(claimDate).getTime(); 

    //UPDATE  the count down every 1 second
    countDownGlobal = setInterval(function () {
        //get todays date n time
        var now = new Date().getTime();

        //find the distance btwn now n count down date
        var distance = countDownDate - now;

        //time calc for days, hrs, min and sec
        ...
        ele.innerHTML = 
            ...
            //if the count down is finished write some texxt
            if (distance < 0) {  
                clearInterval(countDownGlobal);
